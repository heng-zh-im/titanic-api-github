...
minikube kubectl -- create -f ./k8s/postgres-secrets.yaml

# 2, prepare persistant storage volume used by postgres db
minikube kubectl -- create -f ./k8s/postgres-storage.yaml

# 3, deploy postgres db from docker image
minikube kubectl -- create -f ./k8s/postgres-deployment.yaml
...
minikube kubectl -- create -f ./k8s/flask-deployment.yaml
...
echo "Now you can browse the url via http://localhost:"`minikube kubectl -- get svc -l app=flask -o wide | grep -v PORT | awk -F':' '{print $2}' | awk -F'/' '{print $1}'`
